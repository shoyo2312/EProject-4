...
Khác `user-service` (bắt buộc token ở mọi endpoint): ở đây **mọi `GET` đều public**, chỉ `POST`/`DELETE` mới bắt buộc token. Nhưng **có token hay không vẫn ảnh hưởng đến kết quả của `GET`** — xem mục 3.3 và 3.4.
...
`rawFileUrl` **không phải URL client tự nghĩ ra**. Nó là URL trả về từ luồng upload file lên object storage (ngoài phạm vi video-service), và server chặn theo allow-list:
...
### `VideoVisibility`
`PUBLIC` | `PRIVATE`. Đặt lúc đăng và cố định. `PRIVATE` = chỉ chủ video xem được (mục 3.3, 3.4).
...
- Giới hạn theo IP là lý do phải poll giãn dần ở mục 3.1: nhiều thiết bị sau cùng một NAT dùng chung hạn mức này.
...
- Không coi `201` của `POST /` là "video đã đăng xong" — phải poll tới `PUBLISHED` mới có `hlsUrl` để phát (mục 3.1).
...
- Không quên gửi token ở các `GET` khi user đã đăng nhập — sẽ mất video `PRIVATE`/`PROCESSING` của chính họ mà không có lỗi nào báo (mục 3.3, 3.4).
