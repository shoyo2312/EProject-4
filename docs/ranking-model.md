...
`PER_TAG` (100) trong `features.py` là cửa sổ ứng viên của một tag: `FeedServiceImpl.videosTagged` đọc `reco:tag:{tag}` bằng `ZREVRANGE 0..PER_TAG-1`, nên video mang tag của người xem nhưng đã rơi khỏi cửa sổ đó nhận `tag_affinity = 0` khi phục vụ — nó vẫn vào pool được qua nhánh trending. `train.candidate_tags` cắt `video_tags` đúng cửa sổ ấy trước khi join, và cắt **trước** bước lọc theo video của frame, vì cửa sổ là 100 video mới nhất của tag chứ không phải 100 video mới nhất có mặt trong frame. Bỏ bước cắt này thì ứng viên đó có giá trị thật khi huấn luyện và bằng 0 khi phục vụ.
