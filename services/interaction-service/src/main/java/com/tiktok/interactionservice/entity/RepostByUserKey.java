...
@AllArgsConstructor
@PrimaryKeyClass
public class RepostByUserKey implements Serializable {

    @PrimaryKeyColumn(name = "user_id", type = PrimaryKeyType.PARTITIONED)
    private Long userId;

    /**
     * Part of the key, not a plain column: it is what "videos I reposted" is ordered by, and
     * what un-repost needs to address the row it must delete. video_id is a Snowflake, so
    ...
    /** Tie-breaker only, so two reposts in the same millisecond stay two rows. */
