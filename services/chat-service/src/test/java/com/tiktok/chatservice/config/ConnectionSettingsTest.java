...
class ConnectionSettingsTest {
    ...
    void theDatabaseLocationComesFromTheEnvironment() throws Exception {
        ...
        environment.getPropertySources().addFirst(new MapPropertySource("env", Map.of("MONGO_HOST", "mongo.internal", "MONGO_USERNAME", "chat", "MONGO_PASSWORD", "s3cret")));
        new YamlPropertySourceLoader().load("app", new ClassPathResource("application.yml"))
                .forEach(environment.getPropertySources()::addLast);

        assertThat(environment.getProperty("spring.data.mongodb.uri")).contains("chat:s3cret@mongo.internal");
    }
}
