...
class VideoEventConsumerTest {
    ...
    void onMessage_newEvent_recordsTheUpload() throws Exception {
        VideoPublishedEvent event = VideoPublishedEvent.of("vid1", 1L, "My video", null, "s3://raw/vid1.mp4", "PUBLIC", List.of("dance"));
    ...
    void onMessage_withoutAHeader_isTreatedAsAPublication() throws Exception {
        VideoPublishedEvent event = VideoPublishedEvent.of("vid2", 1L, "My video", null, "s3://raw/vid2.mp4", "PUBLIC", List.of());
        givenFirstDelivery(event.eventId());
