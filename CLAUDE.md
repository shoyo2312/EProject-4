...
- Consumer: **claim** `eventId` TRƯỚC khi xử lý, không phải check-rồi-ghi-sau. Check-then-act để lọt 2 delivery song song (rebalance) cùng qua cửa → `$inc` đếm 2 lần, sai vĩnh viễn
  ...
  - Elasticsearch: index `processed_events` với `op_type=create`, 409 nghĩa là đã xử lý (xem `search-service/IdempotentEventProcessor`). Ở đây check-then-act còn hỏng nặng hơn: document vừa ghi chưa đọc lại được cho tới lần refresh kế tiếp (mặc định 1s) nên mọi redelivery trong cửa sổ đó **luôn** lọt qua. `op_type=create` giải quyết ở shard, không phụ thuộc refresh
  - Counter trên Elasticsearch: scripted partial update + `retry_on_conflict`, KHÔNG đọc-sửa-ghi cả document — `save()` ghi đè cả field mà consumer khác vừa đổi (xem `search-service/SearchIndexWriter`)
- Event class lấy từ `libs/event-schema`
- **Topic trộn nhiều event type** (`admin.moderation-events`, `video.video-events`): payload JSON không có field phân biệt loại — dùng Kafka header `eventType` (đọc qua `@Header(name = "eventType")`) để route, KHÔNG suy đoán từ shape JSON. Thiếu route thì Jackson **vẫn parse được** sang class sai với mọi field vắng mặt là null, không exception, không log — service chỉ âm thầm làm sai việc. `video.video-events` mang `VideoPublishedEvent` + `VideoDeletedEvent` + `VideoPurgedEvent` + `VideoVisibilityChangedEvent` cùng key `videoId`, nên Kafka đảm bảo thứ tự. **Xoá video là hai event, không phải một**: `VideoDeletedEvent` phát NGAY khi owner xoá — nó là thứ duy nhất gỡ video khỏi search index và feed recommendation, hoãn nó là để video đã xoá vẫn tìm thấy được; `VideoPurgedEvent` phát khi hết `video.trash.retention` (mặc định 30 ngày) và chỉ media-worker xử lý — đó mới là lúc media bị xoá khỏi MinIO, giữ thùng rác để admin còn xem lại được. Đừng gộp lại làm một. Ngoại lệ: video bị xoá trước khi publication kịp announce vẫn phát cả hai — file raw đã nằm trong MinIO và `VideoPurgedEvent` là thứ duy nhất còn nhắc tới key đó. Mọi consumer của `VideoDeletedEvent`/`VideoPurgedEvent` phải no-op với `videoId` lạ. Consumer coi **header vắng mặt = `VideoPublishedEvent`** (producer đời cũ chỉ gửi loại đó)
